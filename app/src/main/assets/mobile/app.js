const STORAGE_KEY = "aftem_mobile_station_demo_v1";
...
const demoJobs = [
  { id: 1, orderNo: "SIP-20260418-001", customer: "SAMI", project: "CELEBI", qty: 11, m2: 11, stage: "Kesim", route: ["Kesim", "Rodaj", "Temper"] },
  { id: 2, orderNo: "SIP-20260418-002", customer: "YILMAZ", project: "ANKARA", qty: 8, m2: 14.4, stage: "Kesim", route: ["Kesim", "Basım", "Robot"] },
  { id: 3, orderNo: "SIP-20260418-003", customer: "ALTIN", project: "FABRIKA", qty: 5, m2: 7.2, stage: "Rodaj", route: ["Kesim", "Rodaj", "Robot"] }
];
...
function filteredJobs() {
  ...
  return state.jobs.filter(job => {
    ...
    const haystack = `${job.orderNo} ${job.customer} ${job.project}`.toLowerCase();
...
function render() {
  ...
  rows.forEach(job => {
    ...
    node.querySelector(".job-qty").textContent = job.qty;
    node.querySelector(".job-m2").textContent = fmt(job.m2);
...
async function loadFromApi() {
  ...
  try {
    const query = new URLSearchParams({ stage: state.station, q: state.search || "" });
    query.set("stage", stationCode(state.station));
